...
SQL
  ...
  upgraded_count="$(psql_value "$predecessor_database" \
    "SELECT count(*) FROM drizzle.__drizzle_migrations;")"
  [[ "$upgraded_count" == "77" ]] ||
    fail "predecessor upgrade recorded $upgraded_count migrations instead of 77"
  ...
  [[ "$preserved_sentinel" == "ci-predecessor-sentinel|CI predecessor tenant sentinel" ]] ||
    fail "tenant sentinel was not preserved through migrations 0025 through 0076"
...
SQL
  ...
  printf '%s\n' \
    "Predecessor upgrade verification passed: replayed 0000-0024, preserved tenant and audit sentinels, upgraded through 0076, and verified schema/grants/journal types."
else
...
SQL
  ...
  source_migration_count="$(psql_value "$POSTGRES_DB" \
    "SELECT count(*) FROM drizzle.__drizzle_migrations;")"
  [[ "$source_migration_count" == "77" ]] ||
    fail "source database is not fully migrated through 0076 (found $source_migration_count records)"

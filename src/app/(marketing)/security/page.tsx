...
export default function SecurityPage() {
  return (
    <LegalPage eyebrow="Security architecture" title="Controls that fail closed." intro="Business Finlynq separates identity, organization keys, accounting data, and public demo access. This page describes the architecture—not a third-party certification.">
      ...
      <section><h2>Public demo</h2><p>The demo principal is selected only on the server, contains synthetic data, and has no posting, close, recovery, bank, webhook, upload, or security-administration permissions. Speculative browser requests cannot create a demo session.</p></section>
      <section><h2>Current release boundary</h2><p>The hosted demo remains read-only. Production accounting writes stay disabled until encrypted persistence, recovery delivery, off-server backup and key restoration, monitoring, and authorization acceptance tests are completed.</p></section>

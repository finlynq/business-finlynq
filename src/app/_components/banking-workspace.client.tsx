...
function ReconciliationView({ workspace }: { workspace: BankingWorkspaceDto }) {
  ...
  async function addMatch(event: FormEvent<HTMLFormElement>) {
    ...
    const requestKey = matchIdempotencyKey || crypto.randomUUID();
    setMatchIdempotencyKey(requestKey);
    setBusy(true); setMessage(""); setError(false);
    try {
    ...
    {workspace.activeReconciliation && <section className="panel" aria-labelledby="active-reconciliation-title">
      ...
      <div className={styles.form}>
        ...
        {(workspace.permissions.reconcilePrepare || workspace.permissions.reconcileReview) && <div className={styles.actions}>
          ...
          {workspace.activeReconciliation.status === "SUBMITTED" && workspace.permissions.reconcileReview && <button className="primary-button" type="button" disabled={busy} onClick={() => { void transition("REVIEW"); }}>Record authorized review</button>}
          {workspace.activeReconciliation.status === "REVIEWED" && workspace.permissions.reconcileReview && <button className="primary-button" type="button" disabled={busy} onClick={() => { void transition("FINALIZE"); }}>Finalize immutable proof</button>}
        ...
        {((workspace.activeReconciliation.status === "REVIEWED" && workspace.permissions.reconcileReview)
          ...
          && workspace.activeReconciliation.status !== "FINALIZED" && workspace.activeReconciliation.status !== "VOIDED" && <form className={styles.inlineForm} onSubmit={(event) => { void voidReconciliation(event); }}>

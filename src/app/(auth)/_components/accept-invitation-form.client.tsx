...
type Enrollment = AuthenticatorEnrollment & Readonly<{ setupToken: string; organizationName: string }>;
...
export function AcceptInvitationForm() {
  ...
  async function skipAuthenticator() {
    ...
    try {
      ...
      setComplete(true); setEnrollment(null);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Account activation is temporarily unavailable."); }
    finally { setBusy(false); }
  }
  ...
  return (
    <form className={styles.form} onSubmit={accept} noValidate>
      ...
      <button className={styles.submitButton} type="submit" disabled={busy || !invitationToken}>{busy ? "Preparing…" : "Continue and choose security"}</button>
    </form>
  );
}

...
import { SIGNUP_REGIONS, type SignupCountry } from "@/modules/identity/signup-policy";
...
export function SignupForm({
  ...
  const [country, setCountry] = useState<SignupCountry>("CA");
  ...
  function changeCountry(nextCountry: SignupCountry) {
    setCountry(nextCountry);
    setRegion(nextCountry === "CA" ? "ON" : "WA");
  }
  ...
  return (
    <>
      ...
      <form className={styles.form} onSubmit={submit} noValidate>
        ...
        <label><span>Business name</span><input name="organizationName" autoComplete="organization" required minLength={2} maxLength={200} /></label>
        <label><span>Legal entity name</span><input name="entityName" autoComplete="organization" required minLength={2} maxLength={200} /></label>
        <div className={styles.formGrid}>
          <label><span>Country</span><select value={country} onChange={(event) => changeCountry(event.target.value as SignupCountry)}><option value="CA">Canada</option><option value="US">United States</option></select></label>
          <label><span>{country === "CA" ? "Province or territory" : "State"}</span><select value={region} onChange={(event) => setRegion(event.target.value)}>{SIGNUP_REGIONS[country].map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label>
        </div>
        <div className={styles.formGrid}>
          <label><span>Entity code</span><input name="entityCode" defaultValue={country === "CA" ? "CA01" : "US01"} key={country} required maxLength={16} pattern="[A-Za-z0-9][A-Za-z0-9_-]*" /><small>Used in the account key; 0000 is reserved.</small></label>
        ...
        <p className={styles.securityNote}>Your business encryption key is generated and wrapped on the server. The owner stays disabled until email verification, password setup, and authenticator confirmation all finish.</p>
      </form>
    </>
  );
}

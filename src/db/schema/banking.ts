...
export const bankStatementImports = pgTable(
  "bank_statement_imports",
  {
    ...
    importMode: text("import_mode").notNull().default("STATEMENT_BALANCES"),
    statementStartOn: date("statement_start_on").notNull(),
    statementEndOn: date("statement_end_on").notNull(),
    openingBalance: numeric("opening_balance", { precision: 38, scale: 9 }),
    closingBalance: numeric("closing_balance", { precision: 38, scale: 9 }),
    currencyCode: text("currency_code").notNull().references(() => currencyDefinitions.code, { onDelete: "restrict" }),
  ...
  (table) => [
    ...
    check("bank_statement_imports_mode_check", sql.raw("(import_mode = 'STATEMENT_BALANCES' AND opening_balance IS NOT NULL AND closing_balance IS NOT NULL AND reconciliation_session_id IS NOT NULL) OR (import_mode = 'TRANSACTION_EXPORT' AND opening_balance IS NULL AND closing_balance IS NULL AND reconciliation_session_id IS NULL)")),

...
export const bankAccountCutovers = pgTable(
  "bank_account_cutovers",
  {
    ...
    lifecycleEffectiveOn: date("lifecycle_effective_on").notNull(),
    supersedesCutoverId: uuid("supersedes_cutover_id"),
  ...
  (table) => [
    ...
    uniqueIndex("bank_account_cutovers_lineage_version_unique").on(table.organizationId, table.lineageId, table.version),
    uniqueIndex("bank_account_cutovers_org_supersedes_unique").on(table.organizationId, table.supersedesCutoverId),
    ...
    check("bank_account_cutovers_version_check", sql.raw("version > 0")),
    check("bank_account_cutovers_state_check", sql.raw("state IN ('ACTIVE', 'INACTIVE')")),
    ...
    foreignKey({ columns: [table.organizationId, table.supersedesCutoverId], foreignColumns: [table.organizationId, table.id], name: "bank_account_cutovers_org_supersedes_fk" }).onDelete("restrict"),
  ],
);

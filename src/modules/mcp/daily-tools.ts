...
export const DAILY_MCP_TOOLS: readonly McpToolDefinition[] = [
  ...
  createDocumentTool({ name: "finlynq_daily_create_sales_invoice", title: "Create sales invoice draft", description: "Create an idempotent sales-invoice draft with exact invoice facts. Omit fx to resolve automatically: FinLynQ uses an eligible tenant-owned stored direct rate first, then an operator-enabled provider allowed by the organization's policy. If neither yields a permitted observation, the tool fails with FX_RATE_UNAVAILABLE before persistence. Explicit fx evidence remains supported.", kind: "SALES_INVOICE", permission: PERMISSIONS.manageReceivables }),
  editDocumentTool({ name: "finlynq_daily_edit_sales_invoice", title: "Edit sales invoice draft", description: "Create a new immutable version of an existing sales-invoice draft. The exact current version is required. Omit fx for stored-first automatic resolution under the organization's provider policy; no permitted observation fails with FX_RATE_UNAVAILABLE before persistence.", kind: "SALES_INVOICE", permission: PERMISSIONS.manageReceivables }),

...
export const DAILY_MCP_TOOLS: readonly McpToolDefinition[] = [
  ...
  createDocumentTool({ name: "finlynq_daily_create_sales_invoice", title: "Create sales invoice draft", description: "Create an idempotent sales-invoice draft with exact invoice facts. Omit fx to have FinLynQ select the latest enabled tenant-owned direct transaction-to-functional rate effective on or before the accounting date. If none exists, the tool fails with FX_RATE_UNAVAILABLE. Explicit fx evidence remains supported.", kind: "SALES_INVOICE", permission: PERMISSIONS.manageReceivables }),
  editDocumentTool({ name: "finlynq_daily_edit_sales_invoice", title: "Edit sales invoice draft", description: "Create a new immutable version of an existing sales-invoice draft. The exact current version is required. Omit fx to have FinLynQ select an eligible stored direct rate; no eligible rate fails with FX_RATE_UNAVAILABLE.", kind: "SALES_INVOICE", permission: PERMISSIONS.manageReceivables }),
  ...
  createDocumentTool({ name: "finlynq_daily_create_supplier_bill", title: "Create supplier bill draft", description: "Create an idempotent supplier-bill draft with exact invoice facts. Omit fx to have FinLynQ select the latest enabled tenant-owned direct transaction-to-functional rate effective on or before the accounting date. If none exists, the tool fails with FX_RATE_UNAVAILABLE. Explicit fx evidence remains supported.", kind: "SUPPLIER_BILL", permission: PERMISSIONS.managePayables }),
  editDocumentTool({ name: "finlynq_daily_edit_supplier_bill", title: "Edit supplier bill draft", description: "Create a new immutable version of an existing supplier-bill draft. The exact current version is required. Omit fx to have FinLynQ select an eligible stored direct rate; no eligible rate fails with FX_RATE_UNAVAILABLE.", kind: "SUPPLIER_BILL", permission: PERMISSIONS.managePayables }),
  ...
  settlementTool({ name: "finlynq_daily_record_customer_receipt", title: "Record customer receipt", description: "Record an actual customer receipt and allocate it to specific receivable open items. Omit fx to select the latest enabled tenant-owned direct rate effective on or before the settlement date; no eligible rate fails with FX_RATE_UNAVAILABLE. This records settlement evidence; it does not initiate a bank payment.", kind: "CUSTOMER_RECEIPT", permission: PERMISSIONS.settleReceivables }),
  settlementTool({ name: "finlynq_daily_record_supplier_payment", title: "Record supplier settlement", description: "Record supplier settlement evidence against exact payable open items. Omit fx to select the latest enabled tenant-owned direct rate effective on or before the settlement date; no eligible rate fails with FX_RATE_UNAVAILABLE. Use settlementAccountCombinationId and settlementMethod BANK (non-control asset), CORPORATE_CARD, SHAREHOLDER_ADVANCE, EMPLOYEE_REIMBURSEMENT, or OTHER_NON_CASH (non-control liability). Legacy bankAccountCombinationId remains supported for BANK. This does not initiate a transfer or bank payment.", kind: "SUPPLIER_PAYMENT", permission: PERMISSIONS.settlePayables }),

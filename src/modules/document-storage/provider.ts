...
export type CloudFile = { id: string; name: string; mimeType: string; size: number; version: string; etag?: string; parentId: string; folder: boolean };
...
export async function exchangeStorageToken(provider: StorageProvider, input: { code: string; verifier: string } | { refreshToken: string }): Promise<StorageCredentials> {
  ...
  const result = z.object({ access_token: z.string(), refresh_token: z.string().optional(), expires_in: z.number().positive().max(86400), scope: z.string().optional() }).parse(await jsonResponse(await fetch(config.tokenUrl, {
...
const graphFileSchema = z.object({ id, name: z.string().max(1000), size: z.number().nonnegative().safe(), eTag: z.string().optional(), cTag: z.string().optional(),
  ...
  parentReference: z.object({ id: z.string().optional(), driveId: z.string().optional() }).optional(), deleted: z.unknown().optional() });
const googleFields = "id,name,mimeType,size,md5Checksum,version,parents,trashed";
const graphFields = "id,name,size,eTag,cTag,file,folder,parentReference,deleted";
...
function graphFile(raw: unknown): CloudFile {
  ...
  return { id: f.id, name: f.name, mimeType: f.file?.mimeType ?? "folder", size: f.size, version: f.cTag ?? f.eTag ?? "folder",
    etag: f.eTag, parentId: f.parentReference?.id ?? "", folder: Boolean(f.folder) };
}
...
export class CloudDrive {
  ...
  async file(fileId: string): Promise<CloudFile> {
    ...
    return this.provider === "GOOGLE_DRIVE" ? googleFile(raw) : graphFile(raw);
  }
  async children(folderId: string, cursor?: string | null): Promise<{ files: CloudFile[]; cursor: string | null }> {
    ...
    return { files: raw.value.map(graphFile), cursor: raw["@odata.nextLink"] ?? null };
  }
  async folder(parentId: string, name: string): Promise<string> {
    ...
    if (found.ok) {
      const folder = graphFile(await jsonResponse(found));
    ...
    if (created.status === 409) { await created.body?.cancel(); return graphFile(await jsonResponse(await this.request(lookup))).id; }
    return graphFile(await jsonResponse(created)).id;
  }
  async provision(companyName: string, existing?: StorageLocation): Promise<StorageLocation> {
    ...
    if (this.provider === "GOOGLE_DRIVE") {
      ...
      const root = graphFileSchema.parse(await jsonResponse(await this.request("/me/drive/special/approot")));
      baseId = root.id; driveId = id.parse(root.parentReference?.driveId); accountId = driveId;
    }
    if (existing) {
  ...
  async findUpload(folderId: string, stem: string): Promise<CloudFile | null> {
    ...
    for (const extension of ["pdf", "png", "jpg"]) {
      ...
      return graphFile(await jsonResponse(response));
    }
    return null;
  }
  async upload(folderId: string, name: string, mimeType: string, bytes: Buffer): Promise<CloudFile> {
    ...
    if (this.provider === "ONEDRIVE") {
      ...
      return graphFile(await jsonResponse(await this.request(`${this.graphPath()}/items/${encodeURIComponent(folderId)}:/${encodeURIComponent(name)}:/content?${query}`, {
  ...
  async move(file: CloudFile, folderId: string, name: string) {
    ...
    return graphFile(await jsonResponse(await this.request(`${this.graphPath()}/items/${encodeURIComponent(file.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json", ...(file.etag ? { "If-Match": file.etag } : {}) }, body: JSON.stringify({ name, parentReference: { id: folderId }, "@microsoft.graph.conflictBehavior": "fail" }) })));
  }
}

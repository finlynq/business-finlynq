...
describe("CI predecessor-upgrade and restore verification", () => {
  ...
  it("replays exactly 0000-0024 before preserving a tenant sentinel through 0075", () => {
    ...
    expect(migrationJournal.entries.find((entry) => entry.idx === 47)?.tag).toBe(
      "0047_bank_statement_imports",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 48)?.tag).toBe(
      "0048_trusted_browser_mfa",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 49)?.tag).toBe(
      "0049_structured_evidence_mime_types",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 50)?.tag).toBe(
      "0050_entra_sso",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 51)?.tag).toBe(
      "0051_entra_signup",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 52)?.tag).toBe(
      "0052_entra_mfa_assurance",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 53)?.tag).toBe(
      "0053_journal_administrative_controls",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 54)?.tag).toBe(
      "0054_tax_filing_reconciliation",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 55)?.tag).toBe(
      "0055_mcp_account_configuration_controls",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 56)?.tag).toBe(
      "0056_statement_transaction_exports",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 57)?.tag).toBe(
      "0057_asset_prepaid_registers",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 58)?.tag).toBe(
      "0058_asset_register_idempotency",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 59)?.tag).toBe(
      "0059_publish_canada_t2_template",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 60)?.tag).toBe(
      "0060_publish_canada_gst_hst_mapping_v2",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 61)?.tag).toBe(
      "0061_publish_canada_t2_mapping_v2",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 62)?.tag).toBe(
      "0062_tranquil_dragon_man",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 63)?.tag).toBe(
      "0063_illegal_jocasta",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 64)?.tag).toBe(
      "0064_slim_living_lightning",
    );
    ...
    expect(migrationJournal.entries.find((entry) => entry.idx === 71)?.tag).toBe(
      "0071_ticket_60_62_hardening",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 72)?.tag).toBe(
      "0072_ticket_63_66",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 73)?.tag).toBe(
      "0073_tax_filing_configuration_lifecycle",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 74)?.tag).toBe(
      "0074_tax_filing_registration_scopes",
    );
    expect(migrationJournal.entries.find((entry) => entry.idx === 75)?.tag).toBe(
      "0075_tax_filing_scope_constraints",
    );
    ...
    expect(lifecycleScript).toContain('[[ "$upgraded_count" == "76" ]]');
    expect(lifecycleScript).toContain(
      "tenant sentinel was not preserved through migrations 0025 through 0075",
    );

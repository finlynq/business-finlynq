...
vi.mock("@/modules/document-storage/provider", async (original) => ({
  ...
  providerConfiguration: () => ({ redirectUri: "https://stage.business.finlynq.com/api/document-storage/callback/ONEDRIVE" }),
}));
...
describe("storage callback feedback (mocked OAuth)", () => {
  it("returns only a fixed outcome to the configured origin after successful authorization", async () => {
    ...
    expect(response.headers.get("location")).toBe("https://stage.business.finlynq.com/app/settings/documents?storage=connected");
  ...
  it("maps recoverable failures without exposing provider details, credentials, or arbitrary error codes", async () => {
    for (const [code, outcome] of [
      ...
      expect((await callback()).headers.get("location")).toBe(`https://stage.business.finlynq.com/app/settings/documents?storage=${outcome}`);
    }
  });
  it("does not exchange a code after denied consent or without a real session", async () => {
    ...
    expect(denied.headers.get("location")).toBe("https://stage.business.finlynq.com/app/settings/documents?storage=failed");

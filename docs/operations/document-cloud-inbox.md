...
Recursive scan limits are operator-configurable without changing provider permissions:

| Setting | Default | Accepted range | Effect |
| --- | ---: | ---: | --- |
| `DOCUMENT_INBOX_MAX_DEPTH` | 8 | 1–15 | Maximum folder depth below Inbox |
| `DOCUMENT_INBOX_MAX_PROVIDER_CALLS` | 10 | 2–20 | Maximum recursive traversal metadata/list calls per sync request |

A provider page remains capped at 50 children and a sync response at 50 files. The provider-call value must be at least the depth value plus one, so every permitted folder can make progress. Fixed connection-boundary checks run before and after this traversal budget. Throttling retains the exact traversal cursor for a later retry. These variables are part of the rendered app environment and development final-acceptance comparison, so a configuration change cannot be silently accepted against a stale container.

Register exact web callbacks at `/api/document-storage/callback/ONEDRIVE` or `/api/document-storage/callback/GOOGLE_DRIVE` under the configured canonical HTTPS origin. Client registration identifies FinLynQ; users then consent using their own accounts. Use mounted client-secret files. Keep callback queries out of proxy logs by submitting the required `log_skip` policy to `/home/ubuntu/finlynq-shared-edge`; the application deployment does not own that route configuration.
...
Users upload PDF, PNG, JPEG, CSV, TSV, TXT, XLS, or XLSX files up to 2 MiB from the settings page, through `finlynq_daily_upload_inbox_document`, or directly into any folder below the cloud Inbox. EML files can also be placed directly in a connected cloud Inbox or uploaded through MCP. Browser/MCP uploads stream through bounded server memory for validation and scanning; no new cloud original is persisted on the application server. PDFs must have 1–100 pages.

EML processing is intentionally bounded to 64 MIME parts, 20 attachments, eight nesting levels, 64 KiB of headers, 4 MiB of decoded content, and a 100,000-character safe text preview. Raw MIME, HTML, signed URLs, and attachment bytes are not returned. HTML-only bodies are reduced to plain text; nested messages, inline artwork, unsupported formats, unsafe filenames, malformed transfer encodings, and duplicate attachment bodies receive explicit per-part outcomes. Supported attachments are scanned and uploaded to the same inbox with an idempotency key derived from the message SHA-256, part index, and attachment SHA-256. Their encrypted source metadata retains message-item and hash lineage. Re-reading or discovering an identical message reuses the same extracted inbox item. Complete the original EML with `ARCHIVE_ONLY` after review to preserve it as immutable cloud evidence, then claim and link the required extracted invoice or receipt items to the appropriate draft.

Structured-file acceptance checks the filename extension, a narrow set of provider MIME aliases, and the actual content. This includes CSV/TSV reported by OneDrive as `application/vnd.ms-excel`. Text accepts strict UTF-8 (with or without BOM) and BOM-marked UTF-16LE/UTF-16BE. The response reports encoding, detected delimiter, quoting, header row, row counts, line endings, and every preview limit. XLS/XLSX responses list worksheets and return bounded values-only cell previews. Formula text/results are not evaluated; formula cells are replaced with `[FORMULA OMITTED]`. Hyperlink targets are not returned or fetched. XLSX central directories are preflighted for entry count, expansion size, encryption, ZIP64, unsafe paths, VBA, and workbook-level external links before parsing. Macro-enabled or malformed workbooks fail with a stable, actionable storage error. Every accepted original still passes ClamAV and SHA-256 validation.

Tabular files remain `PENDING` and carry `routingTarget: BANKING_IMPORT_REVIEW` instead of being stranded as unsupported `NEEDS_REVIEW` items. An authorized MCP client can now extract and preview a bank or credit-card statement, then submit an explicitly confirmed `IMPORT_STATEMENT` completion. That completion creates immutable bank observations, links the original evidence, and opens a draft reconciliation without posting a journal. It requires a same-company account mapping and the unchanged reviewed preview hash. See the [bank statement import guide](bank-statement-import.md) for signs, permissions, deduplication, confirmation, and recovery.
...
The nine cloud tools are `finlynq_daily_list_document_storage`, `finlynq_daily_upload_inbox_document`, `finlynq_daily_sync_document_inbox`, `finlynq_daily_list_document_inbox`, `finlynq_daily_claim_inbox_document`, `finlynq_daily_read_inbox_document`, `finlynq_daily_complete_inbox_document`, `finlynq_daily_review_inbox_document`, and `finlynq_daily_retry_document_filing`. Sync traverses the Inbox breadth-first and returns at most 50 files per call. It persists an organization-key-encrypted, bounded folder/page cursor; repeat while `hasMore` is true. `restart: true` explicitly discards a stale traversal and begins at the Inbox root. Each response reports `discovered`, `unchanged`, `skipped`, `unsupported`, and `failed` counts plus at most ten safe relative-path issue records. List results include the encrypted-at-rest source relative path and have independent `nextCursor` pagination. Claims expire after ten minutes and can be renewed using the same UUID by the same client. A different browser or MCP connection cannot complete the lease.

Reading returns actual MCP image blocks for PDF/images or deterministic text/workbook preview objects for structured files, together with page/sheet count, SHA-256, source path, and duplicate hints. The client's model interprets that content. FinLynQ makes no model API calls and has no AI API-key setting. The user's client subscription/usage and FinLynQ's hosting, scanning, storage metadata, and bandwidth costs remain separate. There is no autonomous background processing when the client is disconnected.
...
- Each cloud read/upload/sync/filing operation checks its stored root and relevant folder ancestry, including nested Inbox ancestry, folder moves, and cross-drive/shortcut responses. Recursive scans never follow provider shortcuts or arbitrary share links, never cross the configured Inbox, and exclude the configured FinLynQ root/Archive IDs to prevent re-ingestion. Queued folder names are refreshed from the provider when traversal resumes. Stable provider item IDs keep renamed or intra-Inbox moved files on the same inbox record; a changed source path/version invalidates an unfinished claim. Identical names in different folders remain distinct. Deleted queued folders are reported as failed and removed from that traversal; a missing pending file remains visible with its historical source path and fails safely when read. File moves during reads and downloads are rejected. OneDrive downloads manually follow [Graph's short-lived preauthenticated `Location`](https://learn.microsoft.com/en-us/graph/api/driveitem-get-content?view=graph-rest-1.0) only for a narrow set of Microsoft-controlled OneDrive/SharePoint hosts; the Graph credential is never forwarded, further redirects are rejected, and the signed URL is never logged or stored. Metadata-only list/claim/review operations retain organization/module RLS and permission checks. Missing Inbox blocks ingestion while existing Archive evidence remains downloadable; a missing/moved Archive blocks its evidence. Reconnect keeps account and folder IDs and resets stale pagination. No recovery silently creates substitute folders.
...
BUSINESS-FINLYNQ-9/15/37 add mocked tests for MIME aliases, UTF encodings, delimiter/quote handling, preview truncation, XLS/XLSX multiple sheets, formula and hyperlink omission, macro/external-link rejection, ZIP expansion limits, bounded EML plain/HTML/multipart parsing and attachment lineage, three-level recursion, more than 50 files, empty/missing folders, shortcuts, archive/cycle exclusion, moved-folder boundaries, queued-folder renames, wide-folder limit failures, throttling, encrypted cursor persistence, and cursor restart. These tests do not validate a live Microsoft or Google account. Live acceptance must still place a PDF, representative CSV/XLS/XLSX files, and a synthetic EML with a supported receipt attachment in nested OneDrive Inbox folders, exhaust `hasMore`, read every page/sheet/message, complete one draft, archive the original EML, file the other originals, and verify the linked evidence SHA-256 and EML attachment lineage.

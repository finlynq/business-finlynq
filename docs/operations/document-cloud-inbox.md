...
Users upload PDF, PNG, JPEG, CSV, TSV, TXT, XLS, or XLSX files up to 2 MiB from the settings page, through `finlynq_daily_upload_inbox_document`, or directly into any folder below the cloud Inbox. Browser/MCP uploads stream through bounded server memory for validation and scanning; no new cloud original is persisted on the application server. PDFs must have 1–100 pages.
...
BUSINESS-FINLYNQ-9/15 add mocked tests for MIME aliases, UTF encodings, delimiter/quote handling, preview truncation, XLS/XLSX multiple sheets, formula and hyperlink omission, macro/external-link rejection, ZIP expansion limits, three-level recursion, more than 50 files, empty/missing folders, shortcuts, archive/cycle exclusion, moved-folder boundaries, queued-folder renames, wide-folder limit failures, throttling, encrypted cursor persistence, and cursor restart. These tests do not validate a live Microsoft or Google account. Live acceptance must still place a PDF and representative CSV/XLS/XLSX files in nested OneDrive Inbox folders, exhaust `hasMore`, read every page/sheet, complete one draft, file the original, and verify the linked evidence SHA-256.

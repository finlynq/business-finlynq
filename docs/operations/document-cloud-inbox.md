...
- Each cloud read/upload/sync/filing operation checks its stored root and relevant folder ancestry, including folder moves and cross-drive/shortcut responses. File moves during reads and downloads are rejected. Metadata-only list/claim/review operations retain organization/module RLS and permission checks. Missing Inbox blocks ingestion while existing Archive evidence remains downloadable; a missing/moved Archive blocks its evidence. Reconnect keeps account and folder IDs and resets stale pagination. No recovery silently creates substitute folders.

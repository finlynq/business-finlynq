...
## Trusted browsers for login MFA

Trusted-browser login is disabled for every organization by default. An organization administrator with `organization.settings.manage` and a fresh MFA step-up may enable it under **Organization settings** and select 7, 30, or 90 days; 30 days is the default selection. Any policy edit, including a duration change, revokes existing trusted-browser records. This prevents an administrator's new policy from silently extending an earlier grant. The mutation uses the existing organization settings version check and records both an immutable authentication event and the paired tenant business audit/outbox event.

The login form offers **Trust this browser** only after the submitted password has been accepted, the account requires MFA, and the organization policy is enabled. The option remains visible after an incorrect authenticator code so a typing error does not silently clear the user's choice. Enrollment happens atomically with a valid TOTP counter and the new MFA-backed login session. A forged checkbox while policy is disabled creates an ordinary MFA session and no trusted-browser record.

The browser receives a random 32-byte opaque cookie. Deployed environments use the host-only `__Host-business_finlynq_trusted_browser` name with `HttpOnly`, `Secure`, `SameSite=Lax`, `Path=/`, no `Domain`, and an expiry matching the selected policy duration. Local non-TLS development uses the non-`__Host-` name so the flow can be tested over HTTP. PostgreSQL stores only the SHA-256 digest, never the raw token. Each record is bound to the user, organization, active membership, a hashed User-Agent signal, and the user's monotonic credential/MFA security epoch.

A returning user must still submit a valid password. PostgreSQL locks the matching digest and replaces it with a new digest before issuing the session, so concurrent use or replay of the old value can create at most one session. Malformed, missing, expired, revoked, cross-user, cross-organization, cross-membership, epoch-mismatched, or different-User-Agent proofs fail into the ordinary MFA challenge and the rejected browser cookie is cleared. Cross-organization revocation attempts return the same unavailable result as a missing record.

A trusted-browser login skips only the login TOTP prompt. Its session has `auth_method='PASSWORD'` with null `mfa_verified_at` and null `step_up_expires_at`. Period controls, banking changes, organization administration, recovery approval, and every other sensitive mutation therefore continue to require an active factor and a fresh ten-minute MFA step-up.

Users manage active records under **Account & security**. The list shows the server-derived browser label, creation time, last use, and expiry. A user can revoke one record, revoke every record for the current organization, or choose **Sign out all devices**, which revokes every active session and trusted-browser record for that user across organizations. An ordinary single-session logout leaves trusted-browser consent in place. Password change or reset, MFA enrollment/reset/recovery or active-factor replacement, membership suspension/reactivation, role assignment changes, administrator session revocation, policy edits, expiry, and logout-all invalidate the applicable records and append immutable authentication events.

The User-Agent binding is a coarse replay signal rather than cryptographic proof of a physical browser. A copied cookie used with a different User-Agent is revoked, and rotation rejects subsequent reuse. A party that has the user's password, the raw cookie, and an exactly spoofed User-Agent can still present the same bearer proof before the legitimate browser rotates it. Cryptographic resistance to that case requires a non-exportable browser credential such as WebAuthn/passkey device binding and a challenge-response flow. Treat the trusted-browser cookie with the same care as a login session credential.

Focused unit and route tests cover cookie attributes, policy disclosure, opt-in, invalid-code retry, trusted login, rejected proof fallback, same-origin mutations, generic cross-tenant revocation, tenant policy UI, and user controls. `tests/db/trusted-browser.integration.test.ts` is the PostgreSQL-backed suite; it is skipped unless `TEST_DATABASE_URL` is set and uses `TEST_APP_DATABASE_URL` when available to verify direct runtime-role table access is denied. The live database suite proves atomic two-client rotation, replay and tenant boundaries, null MFA/step-up assurance, permission and step-up enforcement for policy changes, every invalidation trigger, and legacy auth-function compatibility. Mocked tests do not replace this PostgreSQL validation.
...
- Replay the complete canonical migration journal through the current head (`0048_trusted_browser_mfa` for this change) on a fresh database, then run the explicit post-migration role reconcilers and test with the non-owner app, authentication worker, and backup roles.

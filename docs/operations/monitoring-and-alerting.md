...
The root-managed systemd monitor and the release-time transitional monitor also
run the protected production external-edge verifier. The deploy-owned cron
fallback cannot read root-only environment files, so it instead verifies the
same immutable central container name, Compose identity, contract-v1 labels,
health, public bindings, and production network directly, followed by the
normal public HTTP and TLS checks. Both paths are read-only and never reload or
modify the central edge.

...
| `migrate` / role reconcilers | Non-root, read-only root | private DB only | all capabilities dropped, one-shot, owner DB credential; app/worker passwords are distinct mounted files | none |
...
- `postgres:16-alpine`, `node:24-alpine`, and the Caddy tag must be resolved to reviewed immutable digests in the release manifest. Renovation of digests must pass the complete gate; tag-only deployments are prohibited for real accounting data.
...
- Compose startup is deliberately gated: canonical migrations must complete, then the explicit app-role grant matrix and worker-role grant matrix must reconcile successfully, and only then may demo bootstrap or the web app start. This same order is exercised after an ACL-free restore.

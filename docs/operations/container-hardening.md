...
| shared edge | Owned and deployed only by `/home/ubuntu/finlynq-shared-edge`; absent from Business Compose | existing Business ingress networks | verified read-only by Business | no Business-owned edge volumes |
| `release_router` | Non-root Caddy, read-only root, bounded tmpfs | frontend, edge, and non-masquerading control networks; loopback-only host port | all capabilities dropped, no-new-privileges, versioned contract/image, private admin socket, graceful drain | non-secret active/maintenance mode sentinel only |
...
| `reset_demo_sandboxes` / `reconcile_demo_sandboxes` | Non-root Node operator, read-only root, bounded tmpfs | private DB only | all capabilities dropped, owner-only one-shot, fixed shared-demo target; no caller-selected tenant | none |
...
Demo reset is the narrow exception that receives migration-owner database access and the organization wrapping key because the shared synthetic baseline contains encrypted data. It has no egress, public port, runtime-role credential, provider key, identity secret, or backup secret. Its CLI rejects all tenant arguments and targets only the fixed public demo.
...
- `postgres:16-alpine` and `node:24-alpine` are committed as readable tag-plus-immutable-index-digest references in the Dockerfile and Compose file. The public Caddy image pin belongs to `/home/ubuntu/finlynq-shared-edge`. Renovating any digest requires resolving it from the official registry and passing the owning repository's complete gate; tag-only deployments are prohibited for real accounting data.

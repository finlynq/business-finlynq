...
| `release_router` | Non-root Caddy, read-only root, bounded tmpfs | frontend and edge networks; loopback-only host port | all capabilities dropped, no-new-privileges, versioned contract/image, private admin socket, graceful drain | non-secret active/maintenance mode sentinel only |

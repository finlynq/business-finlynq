...
- the read-only shared-edge verifier validates contract-v1 central identity and Business production behavior; it never converges or repairs Caddy.

Production and development deployments acquire the shared host deployment lock, preventing application builds and migrations from overlapping on the server. A no-op production check runs the read-only Business public-edge verifier and reports drift; it never repairs or recreates the central proxy or operates on sibling containers, networks, or volumes.

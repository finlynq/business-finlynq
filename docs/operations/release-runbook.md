...
6. only after browser acceptance, recreate the app from the same immutable image ID with the reviewed final gate posture, verify exact app/worker identities, internal readiness, and Docker health, prove maintenance remains in place, reload the existing router live-active without changing its durable maintenance sentinel, then verify final public readiness and the production-scoped external-edge live-uncommitted contract when configured; and

...
5. Bootstrap and reconcile the complete demo pool, install the single nightly reconciliation schedule, and create the mandatory `/etc/business-finlynq/operations.env` with the full release SHA, exact app-gate expectations, `MONITOR_EXPECT_DEMO_MAINTENANCE=true`, pool size 128, and minimum ready capacity 4. After the email/MFA/recovery, real-write, backup/restore, and bank-feed gates are independently evidenced, enable only the corresponding flags and update monitoring expectations in the same release. Real writes additionally require the audited per-organization procedure in [Real-account activation and emergency write disable](../operations/real-account-activation.md); never treat the global flag as tenant approval. The current hosted deployment uses this combined demo/private-account mode; a demo-only installation should leave every real-account flag false.

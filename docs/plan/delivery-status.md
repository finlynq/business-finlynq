...
| G0-05 — operations and incidents | In verification | The implementation now includes aggregate-only internal telemetry/request correlation, commit-addressed release/rehearsal controls, an under-6-hour RPO/4-hour RTO contract with measured restore evidence, read-only audit-graph and request/outbox-lineage verification, and severity plus authentication/write/migration/connector/accounting incident procedures. It is not Done until two clean release rehearsals, a fresh encrypted off-site restore meeting both objectives, external alert-delivery tests, write-shutdown/read-only proof, and the five tabletop exercises are retained and approved. |

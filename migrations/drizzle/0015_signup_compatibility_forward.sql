-- Forward-only compatibility for databases that recorded the first published
-- 0013 before its invitation-precedence hardening was folded into that file.
-- Clean installs already have these columns, so every schema operation here is
-- deliberately safe to apply after the current 0013 as well.

-- BEGIN SIGNUP COMPATIBILITY COLUMN BACKFILL
...
UPDATE auth_organization_signups signup SET
  identity_encryption_user_id = coalesce(
    signup.identity_encryption_user_id, signup.user_id
  ),
  requested_email_ciphertext = coalesce(
    signup.requested_email_ciphertext, selected_user.email_ciphertext
  ),
  requested_display_name_ciphertext = coalesce(
    signup.requested_display_name_ciphertext,
    selected_user.display_name_ciphertext,
    selected_user.email_ciphertext
  )
FROM users selected_user
WHERE selected_user.id = signup.user_id
  AND (
    signup.identity_encryption_user_id IS NULL
    OR signup.requested_email_ciphertext IS NULL
    OR signup.requested_display_name_ciphertext IS NULL
  );
...
BEGIN
  IF EXISTS (
    ...
    WHERE signup.identity_encryption_user_id IS NULL
      OR signup.requested_email_ciphertext IS NULL
      OR signup.requested_display_name_ciphertext IS NULL
  ) THEN
    RAISE EXCEPTION 'Organization signup identity compatibility backfill is incomplete';
  END IF;
END
$$;

ALTER TABLE auth_organization_signups
  ALTER COLUMN identity_encryption_user_id SET NOT NULL,
  ALTER COLUMN requested_email_ciphertext SET NOT NULL,
  ALTER COLUMN requested_display_name_ciphertext SET NOT NULL;
-- END SIGNUP COMPATIBILITY COLUMN BACKFILL
--> statement-breakpoint

-- Replace every signup function whose security/concurrency behavior changed
-- after the first 0013 was published. Keeping the signatures stable upgrades
-- previously migrated databases without granting broader table access.
...
BEGIN
  ...
  -- Dynamic SQL keeps this function compatible when 0014 is not yet present.
